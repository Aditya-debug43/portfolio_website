...
window.saveBrand = function() {
  ...
  const data = {
    ...
    pwd: newPwd || b.pwd || 'admin123',
  };
  saveBrandData(data);
  if (newPwd) sessionStorage.setItem('ag_auth', newPwd);
  applyBrand();
...
window.addItem = async function() {
  ...
  localStorage.setItem(KEYS.port, JSON.stringify(items));
...
window.delItem = async function(id) {
  ...
  localStorage.setItem(KEYS.port, JSON.stringify(filtered));
  renderPortfolio();
  renderAdminPortfolio();
};
...
window.delPriceRow = async function(i) {
  const rows = await loadPricing();
  rows.splice(i, 1);
  localStorage.setItem(KEYS.price, JSON.stringify(rows));
  renderPricing();
  renderAdminPricing();
};
...
window.savePricing = async function() {
  ...
  localStorage.setItem(KEYS.price, JSON.stringify(saved));
  renderPricing();
  renderAdminPricing();
